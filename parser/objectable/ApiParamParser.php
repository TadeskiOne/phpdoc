...
use restdoc\parser\objectable\docElement\ApiParam;
use restdoc\Utils;
...
class ApiParamParser implements ApiParserInterface
{
    ...
    public function parse(string $content, string $source, string $defaultGroup = 'Parameter'): ?AbstractDocElement
    {
        ...
        preg_match_all('/'.$this->getParsedRegExp().'/iu', $content, $matches);
        ...
        if ($allowedValues) {
            ...
            preg_match_all($regExp, $allowedValues, $allowedValuesMatch);

            $allowedValues = $allowedValuesMatch[0];
        }

        // Set global group variable
        $this->group = $matches[1][0] ?: $defaultGroup;
        ...
        return (new ApiParam())
            ...
            ->field($matches[6][0])
            ->defaultValue($matches[7][0] ?? $matches[8][0] ?? $matches[9][0])
            ->description(Utils::unindent($matches[10][0] ?? ''));
    }
