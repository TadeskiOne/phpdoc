...
/**
 * Class ApiGroupWorker
 */
class ApiGroupWorker implements ApiWorkerInterface
{
    private $messages = [
        'common' => [
            'element' => 'apiGroup',
            ...
            foreach ($parsedFile as &$block) {
                ...
                if ($block['global'] === []) {
                    ...
                    if (!$group) {
                        ...
                        $group = realpath($filenames[$parsedFileIndex]);
                    }

                    // replace special chars
                    $group = preg_replace('/[^' . implode('-', $this->specialSymbolsForRegExp) . ']/ui', '_', $group);
